...
describe("scoringService", () => {
  test("calculates expected normalized scores", () => {
    ...
    const scores = calculateScores(grouped, { min: 1, max: 5 });
  ...
  test("validateGroupedAnswers throws on bad input", () => {
    // @ts-ignore
    expect(() => validateGroupedAnswers(null)).toThrow();
    // missing trait
    // @ts-ignore
    expect(() => validateGroupedAnswers({ openness: [1, 2] })).toThrow();
    // non-number in array
    // @ts-ignore
    expect(() =>
      validateGroupedAnswers({
